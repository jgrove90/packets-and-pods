(() => {
  // ns-hugo-imp:/home/jgrove/Projects/packets-and-pods/packetsandpods/themes/FixIt/assets/js/utils/common.js
  ...
  // ns-hugo-imp:/home/jgrove/Projects/packets-and-pods/packetsandpods/themes/FixIt/assets/js/lib/file-tree.js
  var FileTree = class {
  ...
  var FixIt = class {
    constructor() {
      ...
      this.scrollEventSet = /* @__PURE__ */ new Set();
      this.resizeEventSet = /* @__PURE__ */ new Set();
      this.switchThemeEventSet = /* @__PURE__ */ new Set();
      this.clickMaskEventSet = /* @__PURE__ */ new Set();
      this.beforeprintEventSet = /* @__PURE__ */ new Set();
      this.afterprintEventSet = /* @__PURE__ */ new Set();
    ...
    initSearch() {
      ...
      if (!searchConfig || _isMobile && this._searchMobileOnce || !_isMobile && this._searchDesktopOnce)
        return;
      ...
      const location2 = searchConfig.location ?? 0;
          ...
          },
          {
            ...
            source: (query, callback) => {
              ...
              if (searchConfig.type === "algolia") {
                this._algoliaIndex = this._algoliaIndex || algoliasearch(
                ...
                if (!window._index) {
                  fetch(searchConfig.fuseIndexURL).then((response) => response.json()).then((data) => {
                    const options = {
                      isCaseSensitive,
                      findAllMatches,
                      minMatchCharLength,
    ...
    initCopyCode(codeBlock, codePreEl) {
      const copyBtn = codeBlock.dataset.mode === "classic" ? codeBlock.querySelector(".code-header .copy-btn") : codeBlock.querySelector(".code-copy-btn");
    ...
    initToc() {
      ...
      const INDEX_OFFSET = 20 + this.breadcrumbHeight + (document.body.dataset.headerDesktop !== "normal" ? document.getElementById("header-desktop").offsetHeight : 0);
        ...
        forEach(document.querySelectorAll(".mapbox:empty"), ($mapbox) => {
          ...
          if (Array.isArray(markerArray) && markerArray.length > 0) {
            markerArray.forEach((marker) => {
              ...
              new mapboxgl.Marker().setLngLat([markerLng, markerLat]).setPopup(popup).addTo(mapbox);
            });
          }
          if (navigation) {
    ...
    initTypeit(target = document) {
      if (this.config.typeit) {
        ...
        const cursorSpeed = typeitConfig.cursorSpeed || 1e3;
    ...
    getSiteTime = () => {
      ...
      let runTime = (now - run) / 1e3, days = Math.floor(runTime / 60 / 60 / 24), hours = Math.floor(runTime / 60 / 60 - 24 * days), minutes = Math.floor(runTime / 60 - 24 * 60 * days - 60 * hours), seconds = Math.floor((now - run) / 1e3 - 24 * 60 * 60 * days - 60 * 60 * hours - 60 * minutes);
    ...
    initServiceWorker() {
      if (this.config.enablePWA && "serviceWorker" in navigator) {
        navigator.serviceWorker.register("/service-worker.min.js", { scope: "/" }).then(function(registration) {
    ...
    onScroll() {
      ...
      window.addEventListener("scroll", (event) => {
        ...
        for (let event2 of this.scrollEventSet) {
          event2();
        }
        this.oldScrollTop = this.newScrollTop;
      }, false);
